...
#[test]
fn test_xml_name() {
    let xml: &str = r#"<name source="ns1" target="nt1" />"#;
    println!("{}", xml);
    let parsed: Name = from_str(xml).expect("Failed to parse XML");
    assert_eq!(parsed.source, "ns1");
    assert_eq!(parsed.target, "nt1");
}
...
fn test_xml_field() {
    ...
    let parsed: Field = from_str(xml).expect("Failed to parse XML");
    assert_eq!(parsed.name.source, "ns1");
    assert_eq!(parsed.name.target, "nt1");

    assert_eq!(parsed.field_type.source, "ts1");
    assert_eq!(parsed.field_type.target, "tt1");
}

#[test]
fn test_xml_parsing1() {
    println!("XML_DATA1 in Docker:");
    println!("{}", XML_DATA1);
