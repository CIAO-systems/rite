...
use crate::com::atoss::atc::protobuf::{
    field::Value as ATC_Value, DoubleCollection, DurationCollection, IntCollection,
    StringCollection, TimestampCollection,
};
...
fn test_add_fields_filter() {
    ...
    let fields = String::from("example_field");

    add_fields_filter(fields.clone(), &mut parameter_meta_data);

    let expected_field = Field {
        name: "fields".to_string(),
        value: Some(StringValue(fields.clone())),
    };
