...
#[test]
fn test_absences_filter_load_success() {
    ...
    let filter = AbsencesFilter::load(&config).expect("Failed to load filter");
    assert_eq!(filter.start_date, Some("2024-01-01".to_string()));
    assert_eq!(filter.end_date, Some("2024-01-31".to_string()));
...
#[test]
fn test_absences_filter_load_missing_optional_values() {
...
#[test]
fn test_absences_filter_invalid_employee_ids() {
    let mut config = Configuration::new();
